...
public class MyReplyPagingCreatorDTO {
	...
	public MyReplyPagingCreatorDTO(long replyTotalByBno, MyReplyPagingDTO myReplyPaging, 
		...
		this.pagingNumCnt = 5; 
		//계산된 끝-시작 페이징 번호: 
		this.endPagingNum = (int)( Math.ceil(myReplyPaging.getPageNum() / (this.pagingNumCnt * 1.0) ) ) * this.pagingNumCnt ; this.startPagingNum = this.endPagingNum - (this.pagingNumCnt -1); 
		//행 총수를 기준으로 한 실제 마지막 페이지 번호 
		lastPageNum = (int)( Math.ceil( (replyTotalByBno * 1.0) / myReplyPaging.getRowAmountPerPage() ) ); 
